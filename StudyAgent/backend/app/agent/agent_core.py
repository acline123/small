...
from app.agent.prompt import build_rag_prompt, build_tool_prompt, format_search_results
from app.llm.deepseek import chat
from app.rag.vectorstore import similarity_search
from app.tools import registry


def handle_chat(session_id: str | None, message: str) -> dict:
    """
    Agent 主流程：
    用户输入 → 意图识别 → 工具调用/RAG → DeepSeek 生成 → 保存历史
    """
    if not session_id:
    ...
    if intent == "search":
        ...
        tool = registry.get("summary_document")
